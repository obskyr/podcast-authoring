...
@REM Ogg version. Stereo because libopus for *some reason* makes *larger* mono files than stereo files. 
...
ffmpeg -i "%~1" -i "%~dp1ffmetadata.txt" -r %SAMPLE_FREQUENCY% -b:a %BITRATE% -vbr off -c:a libopus -map_metadata 0 -map_metadata 1 -y "%~dp1%~n1.ogg"
